...
- **Unclaimed** only lists customers who actually need a human — i.e. they requested a human (a handoff alert) or AI was paused for that specific customer. Turning a store's **Global AI** master switch off just stops the bot replying; it no longer dumps every conversation in that store into Unclaimed. Agents can still **Take over** any AI conversation from the thread view.
